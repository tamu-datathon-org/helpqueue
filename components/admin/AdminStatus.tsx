...
export default function AdminStatus(props : {
  ...
  return (<div>
    <div className="flex items-center flex-shrink-0">
      <a onClick={() => toggleStatus("mentor", props.user.mentor)} className={`${
        props.user.mentor
          ? "bg-blue-500 text-white border-2 border-blue-500"
          : "border-2 border-gray-300"} py-2 px-4 mr-2 rounded-md text-xs sm:text-sm font-medium cursor-pointer`}>
        MENTOR
      </a>
      <a onClick={() => toggleStatus("admin", props.user.admin)} className={`${
        props.user.admin
          ? "bg-blue-500 text-white border-2 border-blue-500"
          : "border-2 border-gray-300"} py-2 px-4 rounded-md text-xs sm:text-sm font-medium cursor-pointer`}>
        ADMIN
      </a>
    </div>
  </div>);
