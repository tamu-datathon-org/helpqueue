...
export default function TicketStream(props : {
  ...
  const {data: ticketsData, error: ticketError, isLoading: isTicketLoading} = useSWR(`/help/api/tickets/${props.filter || "all"}`, fetcher, {refreshInterval: 5000});

  if (isTicketLoading) {
    return (<div className="flex justify-center p-8 bg-white border border-gray-100 shadow-md rounded-xl my-8">
  ...
  ticketsData.tickets.map((ticket : Ticket, index : number) => {
    ticketList.push(<div key={index} className="relative block p-4 sm:p-8 bg-white border border-gray-100 shadow-md rounded-xl my-8 md:w-[90vw] lg:w-[35vw] 2xl:w-[500px]">
